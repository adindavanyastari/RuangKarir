    ...
    <div class="max-w-7xl mx-auto px-4">
        ...
        <div class="mb-8">
            <div class="flex justify-between items-center">
                <div>
                    <h1 class="text-3xl font-bold text-slate-800">Kelola Lowongan Magang</h1>
                    <p class="text-slate-600 mt-2">Kelola semua lowongan magang yang telah dibuat</p>
                </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            <div class="bg-white rounded-xl shadow-md p-6 border-l-4 border-blue-500">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-bold text-gray-900">{{ $internships->total() ?? 0 }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-md p-6 border-l-4 border-green-500">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-bold text-gray-900">{{ $internships->where('is_active', 1)->count() ?? 0 }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-md p-6 border-l-4 border-yellow-500">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <p class="text-sm font-medium text-gray-500">Segera Berakhir</p>
                        <p class="text-2xl font-bold text-gray-900">{{ $internships->where('deadline_pendaftaran', '<=', now()->addDays(7))->count() ?? 0 }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-md p-6 border-l-4 border-red-500">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-bold text-gray-900">{{ $internships->where('is_active', 0)->count() ?? 0 }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        @if($internships->count() > 0)
            <div class="bg-white rounded-xl shadow-lg overflow-hidden">
                <div class="px-6 py-4 border-b border-gray-200">
                    <h3 class="text-lg font-semibold text-gray-900">Daftar Lowongan Magang</h3>
                </div>
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Perusahaan & Posisi</th>
                            ...
                            @foreach($internships as $internship)
                                <tr class="hover:bg-gray-50">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div>
                                            <div class="text-sm font-medium text-gray-900">{{ $internship->nama_perusahaan }}</div>
                                            <div class="text-sm text-gray-500">{{ $internship->posisi_magang }}</div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-900">{{ ucfirst($internship->lokasi_magang) }}</div>
                                        <div class="text-sm text-gray-500">{{ $internship->durasi_magang }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-900">{{ $internship->deadline_pendaftaran->format('d M Y') }}</div>
                                        @php
                                            $daysLeft = now()->diffInDays($internship->deadline_pendaftaran, false);
                                        @endphp
                                        @if($daysLeft > 0)
                                            <div class="text-xs text-green-600">{{ $daysLeft }} hari lagi</div>
                                        @else
                                            <div class="text-xs text-red-600">Berakhir</div>
                                        @endif
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
        ...
        @else
            ...
            <div class="bg-white rounded-xl shadow-lg p-12 text-center">
                ...
                <p class="text-gray-600 mb-6">Mulai dengan membuat lowongan magang pertama Anda.</p>
                <a href="{{ route('admin.internships.create') }}" 
