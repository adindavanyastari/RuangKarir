                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                        ...
                        <div>
                            ...
                            <select name="semester"
                                ...
                                <option value="1-2" {{ request('semester') == '1-2' ? 'selected' : '' }}>Semester 1-2
                                </option>
                                <option value="3-4" {{ request('semester') == '3-4' ? 'selected' : '' }}>Semester 3-4
                                </option>
                                <option value="5-6" {{ request('semester') == '5-6' ? 'selected' : '' }}>Semester 5-6
                                </option>
                                <option value="7-8" {{ request('semester') == '7-8' ? 'selected' : '' }}>Semester 7-8
                                </option>
                            </select>
                        </div>
                    </div>
            ...
            @if ($otherProfiles->count() > 0)
                <div class="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-6 mb-8">
                    @foreach ($otherProfiles as $otherProfile)
                        <article
                            class="bg-white rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden border border-orange-200 group">
                            <!-- Profile Header -->
                            <div class="p-6 pb-4">
                                <div class="md:flex items-center mb-4">
                                    <div
                                        class="w-16 h-16 rounded-full overflow-hidden bg-gray-200 mr-4 flex-shrink-0 ring-2 ring-orange-100 mb-2 md:mb-0">
                                        @if ($otherProfile->foto && $otherProfile->foto !== 'icon')
                                            <img src="{{ asset($otherProfile->foto) }}" alt="Profile Photo"
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <h3 class="text-lg font-bold text-gray-900 truncate">{{ $otherProfile->nama }}
                                        </h3>
                                        <p class="text-sm text-orange-600 font-semibold truncate">
                                            {{ $otherProfile->prodi }}</p>
                                        <p class="text-xs text-gray-500">Semester {{ $otherProfile->semester }} •
                                            {{ $otherProfile->fakultas }}</p>
                                    </div>
                                </div>
                                    ...
                                    <div class="mb-4">
                                        ...
                                        <div class="flex flex-wrap gap-1">
                                            @foreach (array_slice(explode(',', $otherProfile->hard_skills), 0, 3) as $skill)
                                                <span
                                                    class="bg-orange-100 text-orange-800 px-2 py-1 rounded-full text-xs font-medium">{{ trim($skill) }}</span>
                                            @endforeach
                                            @if (count(explode(',', $otherProfile->hard_skills)) > 3)
                                                <span
                                                    class="text-xs text-gray-500 bg-gray-100 px-2 py-1 rounded-full">+{{ count(explode(',', $otherProfile->hard_skills)) - 3 }}
                                                    more</span>
                                            @endif
                                        </div>
                                    </div>
                                ...
                                @if ($otherProfile->ringkasan_pribadi)
                                    <p class="text-sm text-gray-600 mb-4 line-clamp-2 leading-relaxed">
                                        {{ Str::limit($otherProfile->ringkasan_pribadi, 100) }}</p>
                                @endif
                            </div>
                ...
                <div class="flex justify-center">
                    <div class="bg-white rounded-2xl shadow-lg border border-orange-200 p-4">
                        <div class="flex flex-wrap md:flex-row items-center justify-center gap-2">
                            {{-- Previous Page Link --}}
                            @if ($otherProfiles->onFirstPage())
                                <span
                                    class="order-2 md:order-1 px-4 py-2 text-gray-400 bg-gray-100 rounded-lg cursor-not-allowed flex items-center">
                            ...
                            @else
                                <a href="{{ $otherProfiles->previousPageUrl() }}"
                                    class="order-2 md:order-1 px-4 py-2 text-orange-600 bg-orange-50 hover:bg-orange-100 rounded-lg transition-all duration-300 flex items-center font-medium border border-orange-200 hover:border-orange-300">
                                    <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M15 19l-7-7 7-7"></path>
                                    </svg>
                            ...
                            <div class="flex flex-wrap justify-center gap-2 order-1 md:order-2 w-full md:w-auto">
                                ...
                                @for ($i = $start; $i <= $end; $i++)
                                    @if ($i == $otherProfiles->currentPage())
                                        <span
                                            class="px-4 py-2 text-white bg-gradient-to-r from-orange-500 to-orange-600 rounded-lg font-bold shadow-md">
                                            {{ $i }}
                                        </span>
                                    @else
                            ...
                            @if ($otherProfiles->hasMorePages())
                                <a href="{{ $otherProfiles->nextPageUrl() }}"
                                    class="order-3 md:order-3 px-4 py-2 text-orange-600 bg-orange-50 hover:bg-orange-100 rounded-lg transition-all duration-300 flex items-center font-medium border border-orange-200 hover:border-orange-300">
                                    Next
                            ...
                            @else
                                <span
                                    class="order-3 md:order-3 px-4 py-2 text-gray-400 bg-gray-100 rounded-lg cursor-not-allowed flex items-center">
                                    Next
                                    <svg class="w-4 h-4 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M9 5l7 7-7 7"></path>
